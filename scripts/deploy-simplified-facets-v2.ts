...
async function main() {
    console.log("🚀 Deploying Simplified Facets V2...\n");
    ...
    try {
        ...
        for (const facetAddress of facetsToRemove) {
            try {
                ...
                if (selectors.length > 0) {
                    // Create a new array copy to avoid readonly issues
                    const selectorsToRemove = [...selectors];
        ...
        console.log("\n🧪 Quick Test...");
        const viewTest = await ethers.getContractAt("GridottoViewFacet", diamondAddress);
        const isPaused = await viewTest.isPaused();
        console.log("Contract paused:", isPaused);
        
        const drawCounter = await viewTest.getDrawIdCounter();
        console.log("Draw ID counter:", drawCounter.toString());
