...
/**
 * @title GridottoAdminFacetV2
 * @notice Fixed version of AdminFacet that uses correct storage
 */
contract GridottoAdminFacetV2 {
    ...
    event NextDrawIdFixed(uint256 newValue);
    ...
    // FIXED: Get next draw ID - now uses correct storage
    function getNextDrawId() external view returns (uint256) {
        return LibGridottoStorageV2.layout().nextDrawId;
    }
    
    // NEW: Force set nextDrawId for fixing issues
    function forceSetNextDrawId(uint256 _value) external onlyOwner {
        require(_value > 0, "Invalid value");
        LibGridottoStorageV2.layout().nextDrawId = _value;
        emit NextDrawIdFixed(_value);
    }
